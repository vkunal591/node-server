...
export const get = asyncHandler(async function (req, res, _next) {
  ...
  const data = await SubcategoryService.get(id, filter);
  sendResponse(httpStatus.OK, res, data, "Record fetched successfully");
});

export const create = asyncHandler(async function (req, res, _next) {
  const createdDoc = await SubcategoryService.create(req.body);
  sendResponse(
    httpStatus.CREATED,
    res,
    createdDoc,
...
export const update = asyncHandler(async function (req, res, _next) {
  const { id } = req.params;
  const updatedDoc = await SubcategoryService.update(id, req.body);
...
export const deleteData = asyncHandler(async function (req, res, _next) {
  const { id } = req.params;
  const deletedDoc = await SubcategoryService.deleteDoc(id);
